...
public:
    ...
    _pcg = pcg;
		...
		_attention.init(&model.att_params, opts.dropProb);
		_guide.init(opts.hiddenSize, -1);
		_avg_pooling.init(opts.hiddenSize, -1);
		_max_pooling.init(opts.hiddenSize, -1);
		_min_pooling.init(opts.hiddenSize, -1);
		_concat.init(opts.hiddenSize * 1, -1);
...
public:
    ...
    _pcg->train = bTrain;
		...
		_guide.forward(_pcg, 0);
		_attention.forward(_pcg, getPNodes(_hidden, words_num), &_guide);
		_neural_output.forward(_pcg, &_attention._output);
